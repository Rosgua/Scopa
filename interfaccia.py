...
def handle_player_turn(card_str_choice):
    ...
    try:
        ...
        if result == 'Scopa':
            add_message(f"Risultato della mossa: {result}")
...
if not st.session_state.game_started:
   ...
   # 1. Area per i messaggi (in alto)
    ...
    # Controlla la fine della partita ad ogni re-run
    if not gs.deck.cards and not gs.playerAI.handCard and not gs.Hplayer.handCard and st.session_state.game_started:
        end_game_logic()
        st.rerun() # <--- SOSTITUITO con st.rerun()
